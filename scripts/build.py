...
def creation_campaign(lang, platform='iphone'):
 ...
 return f'<section class="creation-campaign"><div><span class="eyebrow">WORLDCOPY / CREATE</span><h2>{title}</h2><p>{lead}</p><ol>'+''.join(f'<li>{e(x)}</li>' for x in stages)+f'</ol><p class="campaign-note">'+('AI 建模与贴图为 Beta 功能，使用前需下载对应引擎；可用性取决于设备。' if zh else 'AI modeling and texturing are Beta features. Download the required engines before use; availability depends on your device.')+f'</p></div><div class="creation-phones">{image(f"campaign/{lang}-02-teddy.png", "真实泰迪熊模型预览" if zh else "Actual teddy model preview", "campaign-phone")}{image(f"campaign/{lang}-03-cat.png", "真实招财猫贴图预览" if zh else "Actual lucky cat texture preview", "campaign-phone")}</div></section><section class="pro-story"><span class="eyebrow">WORLDCOPY PRO</span><h2>'+('创作自由，交付专业。' if zh else 'Create freely. Deliver professionally.')+f'</h2><p>{free}</p><p>{pro}</p><a class="text-link" href="{url(lang,"support/")}">'+('了解设备与权益 →' if zh else 'Explore capabilities and access →')+f'</a></section><section class="campaign-gallery">{gallery}</section>'
