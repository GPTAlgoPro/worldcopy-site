...
def page(lang,path,title,body):
 ...
 nav=''.join(f'<a {"aria-current=\"page\"" if path==p else ""} href="{url(lang,p)}">{n}</a>' for p,n in zip(PAGES[:4],NAV[lang][:4]))
 ...
 h=f'''<!doctype html><html lang="{'zh-Hans' if zh else 'en'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="{description}"><meta name="theme-color" content="#f4f3ee"><title>{e(title)} · WorldCopy 摹界</title><link rel="canonical" href="{ORIGIN}{url(lang,path)}"><link rel="alternate" hreflang="zh-Hans" href="{ORIGIN}{url('zh',path)}"><link rel="alternate" hreflang="en" href="{ORIGIN}{url('en',path)}"><link rel="icon" href="{BASE}assets/app-icon.png"><link rel="stylesheet" href="{BASE}style.css"></head><body><a class="skip" href="#main">{'跳转正文' if zh else 'Skip to content'}</a><header><a class="brand" href="{url(lang)}">{image('app-icon.png','WorldCopy','icon',False)}<span>{"摹界" if zh else "WorldCopy"} <b>{"WorldCopy" if zh else "摹界"}</b></span></a><nav aria-label="{'主导航' if zh else 'Main navigation'}">{nav}</nav><a class="language" href="{url('en' if zh else 'zh',path)}" lang="{'en' if zh else 'zh-Hans'}">{'EN' if zh else '中文'}</a></header><main id="main">{body}</main><footer><div><a class="wordmark" href="{url(lang)}">WorldCopy<span> / 摹界</span></a><p>{'摹写真实，塑造想象。' if zh else 'Capture reality. Shape imagination.'}</p><small>© 2026 Kai Sun</small></div><div class="footer-links">{footer}<a href="https://github.com/GPTAlgoPro/worldcopy-site">{'网站项目' if zh else 'Website project'} ↗</a></div></footer></body></html>'''
 (dest/'index.html').write_text(h)

for lang in ['zh','en']:
 zh=lang=='zh'
 phone='ios-model-zh.png' if (ROOT/'assets/ios-model-zh.png').exists() else 'ios-home-zh.png'
 hasphone=(ROOT/'assets'/phone).exists()
 phoneimg=image(phone,'摹界 iPhone 实际界面' if zh else 'WorldCopy on iPhone, Chinese interface','phone-screen',False) if hasphone else ''
 ...
 body+='<section class="gallery">'+figure('mac-library-zh.png','Mac 模型库，包含扫描与 AI 作品' if zh else 'Mac library with scanned and AI-generated models','扫描作品与 AI 作品，保存在同一个模型库。' if zh else 'Scanned and AI-generated work, together in one library. Chinese app interface shown.')+'</section>'
 ...
 if hasphone:body+='<section class="phone-gallery">'+figure(phone,'iPhone 上的摹界实际界面' if zh else 'Actual WorldCopy iPhone interface','iPhone 实机界面' if zh else 'Actual iPhone capture; Chinese interface shown.','phone-detail')+'</section>'
 body+=cards(items)
 body+=f'<section class="requirements"><h2>{"设备与资源" if zh else "Devices & resources"}</h2><p>{"需要 iOS / iPadOS 26 或更高版本。扫描和 AR 以当前设备运行时支持情况为准；iPad 并不因此具备与 iPhone 相同的扫描能力。AI 建模与 PBR 需要对应引擎资源，以及足够的实时可用内存和磁盘空间。温度、地区和资源状态也会影响可用性。" if zh else "Requires iOS / iPadOS 26 or later. Scanning and AR depend on runtime device support; iPad does not necessarily offer the same capture capabilities as iPhone. AI modeling and PBR require the corresponding engine resources and sufficient available memory and storage. Temperature, region and resource status also affect availability."}</p><p>{"支持简体中文和英文。AI 结果及处理时间因输入和设备而异，生成模型不应当作实物测量结果。" if zh else "Available in Simplified Chinese and English. AI results and processing time vary with input and device. Generated geometry is not a measurement of the real object."}</p></section>'
 page(lang,'ios/','iOS 版' if zh else 'For iOS',body)
 # Mac product
 body=f'<section class="page-top"><span class="eyebrow">WORLDCOPY / macOS</span><h1>{"给你的作品，<br><em>更大的发挥空间。</em>" if zh else "More room<br><em>for your next creation.</em>"}</h1><p class="lead">{"从扫描包重建到 AI 创作，把编辑、检查和交付放进同一个工作区。" if zh else "From capture reconstruction to AI creation, bring editing, inspection and delivery into one workspace."}</p></section><section class="gallery">'+figure('mac-model-zh.png','Mac 上的 AI 模型预览' if zh else 'An AI-generated model in the Mac workspace','Mac 实际界面 · AI 生成作品' if zh else 'Actual Mac interface · AI-generated model · Chinese interface shown.')+'</section>'
 ...
 body+='<section class="gallery">'+figure('mac-library-zh.png','Mac 模型库' if zh else 'WorldCopy Mac library','在模型库中重新打开作品，继续处理。' if zh else 'Reopen your saved work and continue. Chinese interface shown.')+'</section>'
